...
foo (bool ssl, const string& path, const string& query) {
  string a = ssl ? "(make this string long enough)       https" : "(make this string long enough)       http";
  a = a + "(make this string long enough)        ://" + path + "(make this string long enough)       /" + query;
  return a;
}
