...
void print_usage(const char *pname) {
  std::cout << "Usage: '" << pname << " <POLICY> <NUM> <NREP = 1>'" << std::endl;
  std::cout << "\twhere policy is 0 for seq, 1 for par, 2 for par_unseq" << std::endl;
  std::cout << "\t      num is number of elements to sort (less then 2^31)" << std::endl;
  std::cout << "\t      nrep is number of cycles to repeat (less then 2^31)" << std::endl;
}
...
config_t read_config(int argc, char **argv) {
  ...
  if ((cfg.policy < 0) || (cfg.policy > 2) || (cfg.num < 1) || (cfg.ncycles < 1)) {
    print_usage(argv[0]);
    exit(0);
  }
...
void random_fill(It start, It fin) {
...
template <typename It>
void dump_range(It start, It fin) {
...
void do_bench(T policy, It start, It fin) {
  std::atomic_thread_fence(std::memory_order_seq_cst);
  std::transform(policy, start, fin, start, [](auto s) { return -s; });
}
...
int main(int argc, char **argv) {
  ...
  auto tfin = chrc::now();
  std::cout << "Wall time: " << duration_cast<cms>(tfin - tstart).count() << std::endl;
}
