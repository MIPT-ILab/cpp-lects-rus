...
void print_usage(const char *pname) {
  ...
  std::cout << "\twhere policy is 0 for seq, 1 for par, 2 for par_unseq, 3 for self" << std::endl;
...
config_t read_config(int argc, char **argv) {
  ...
  if ((cfg.policy < 0) || (cfg.policy > 3) || (cfg.num < 1) || (cfg.ncycles < 1)) {
    print_usage(argv[0]);
    exit(0);
  }
...
T parallel_accumulate(Iterator first, Iterator last, T init = 0) {
  ...
  for (; length >= bsize * (tidx + 1); first += bsize, tidx += 1)
    threads[tidx] = std::thread(accumulate_block, 
...
auto do_bench(T policy, It start, It fin) {
