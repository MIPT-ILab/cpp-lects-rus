...
public:
  ...
  iterator(concat_view<V1, V2> &parent) : 
    parent_(&parent),
    ifst_(ranges::begin(parent.fst_)), 
    isnd_(ranges::begin(parent.snd_)) {}

  value_type operator*() const {
    return !first_done_ ? *ifst_ : *isnd_;
  }
...
template <ranges::view V1, ranges::view V2>
typename concat_view<V1, V2>::iterator concat_view<V1, V2>::begin() {
    return concat_view<V1, V2>::iterator{*this};
}

template <ranges::view V1, ranges::view V2>
typename concat_view<V1, V2>::sentinel concat_view<V1, V2>::end() {
    return concat_view<V1, V2>::sentinel{ranges::end(snd_)};
}

template <ranges::viewable_range R1, ranges::viewable_range R2>
auto concat(R1 &&r1, R2 &&r2) {
  return concat_view<views::all_t<R1>, views::all_t<R2>>(
    views::all(std::forward<R1>(r1)), views::all(std::forward<R2>(r2))
  );
};

#if 0
template <ranges::view V1, ranges::view V2>
auto concat(V1 &&v1, V2 &&v2) {
  return concat_view<V1, V2>(
...
int main() {
  ...
  auto others = views::iota(5) | views::filter(modulo_six) | views::filter(is_prime);
